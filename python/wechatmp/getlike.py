...
s = requests.session()
headers = {
    'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 9_1 like Mac OS X) AppleWebKit/601.1.txt.46 (KHTML, like Gecko) Version/9.0 Mobile/13B143 Safari/601.1.txt'}
data = {
    'is_only_read': 1,
...
def get_article(fakeid, appmsg_token):
    begin = 0
    while 1:
        url = 'https://mp.weixin.qq.com/mp/profile_ext?action=getmsg&__biz=%s&f=json&offset=%s&count=10&uin=%s&key=%s&appmsg_token=%s' % (
            quote(fakeid), begin, UIN, KEY, appmsg_token)
        print
        url
        try:
...
def get_cookie(biz):
    ...
    appmsg_token = re.findall('window.appmsg_token = "([^"]+)', page)[0]
    return appmsg_token
...
def get_likes(url):
    ...
    try:
        ...
        comm_url = 'https://mp.weixin.qq.com/mp/appmsg_comment?action=getcomment&scene=0&__biz=%s&comment_id=%s&offset=0&limit=1.txt&appmsg_token=%s&f=json' % (
...
def getart(dic):
    for item in dic:
        ...
        # id = item['id']
        getarg()
        appmsg_token = get_cookie(fakeid)
        for each in get_article(fakeid, appmsg_token):
