...
import com.dbn.cloud.platform.security.entity.DefaultClientDetails;
...
/**
 * 将cloud_oauth2_client_details表数据缓存到redis，这里做个缓存优化
 * 模块中有对cloud_oauth2_client_details的crud， 注意同步redis的数据
 * 注意对cloud_oauth2_client_details清楚redis db部分数据的清空
 */
@Slf4j
...
public class RedisClientDetailsService extends JdbcClientDetailsService {
    ...
    private static final String SELECT_CLIENT_DETAILS_SQL = "select client_id, client_secret, resource_ids, scope, authorized_grant_types, " +
            ...
            "from cloud_oauth2_client_details where client_id = ? and status = 1  ";
    ...
    private static final String SELECT_FIND_STATEMENT = "select client_id, client_secret,resource_ids, scope, "
            ...
            + "refresh_token_validity, additional_information, autoapprove ,if_limit, limit_count ,id  from cloud_oauth2_client_details where status = 1 order by client_id ";
    ...
    /**
     * 将cloud_oauth2_client_details全表刷入redis
     */
    public void loadAllClientToCache() {
        if (redisTemplate.hasKey(UaaConstant.CACHE_CLIENT_KEY)) {
            return;
        }
        log.info("将cloud_oauth2_client_details全表刷入redis");

        List<ClientDetails> list = this.listClientDetails();
        if (CollectionUtils.isEmpty(list)) {
            log.error("cloud_oauth2_client_details表数据为空，请检查");
            return;
        }
    ...
    private static class ClientDetailsRowMapper implements RowMapper<ClientDetails> {
        private com.dbn.cloud.auth.uua.json.JsonMapper mapper = createJsonMapper();
     ...
     * @return
     */
    private static com.dbn.cloud.auth.uua.json.JsonMapper createJsonMapper() {
        if (ClassUtils.isPresent("org.codehaus.jackson.map.ObjectMapper", null)) {
            return new com.dbn.cloud.auth.uua.json.JacksonMapper();
        } else if (ClassUtils.isPresent("com.fasterxml.jackson.databind.ObjectMapper", null)) {
            return new com.dbn.cloud.auth.uua.json.Jackson2Mapper();
        }
        return new com.dbn.cloud.auth.uua.json.NotSupportedJsonMapper();
    }
